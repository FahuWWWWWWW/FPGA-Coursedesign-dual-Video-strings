...
wire [15:0] fsmc_tx_data_13; // 0x000D: 当前图像处理模式 (回读)
wire [15:0] fsmc_tx_data_14; // 0x000E: 当前二值化阈值 (回读)
reg [15:0] fsmc_tx_data_15;
reg [15:0] fsmc_tx_data_16; // 0x0010: 帧计数器的低16位 (Frame Cnt)
reg [15:0] fsmc_tx_data_17; // 0x0011: 有效像素计数低16位
reg [15:0] fsmc_tx_data_18; // 0x0012: 有效像素计数高16位
reg [15:0] fsmc_tx_data_19; // 0x0013: 调试位标记 (Reserved)

wire [63:0] debug_data;      // 调试信号

assign fsmc_tx_data_13 = {13'd0, image_algo_mode};
assign fsmc_tx_data_14 = {8'd0, image_threshold};
...
always @(posedge FSMC_CLK or negedge rst_n) begin
    if (!rst_n) begin
        ...
        fsmc_tx_data_16 <= 16'd0;
        fsmc_tx_data_17 <= 16'd0;
        fsmc_tx_data_18 <= 16'd0;
        fsmc_tx_data_19 <= 16'hFFFF;
    end else begin
        ...
        // [0x000D] 图像处理模式 (assigned as wire)
        // fsmc_tx_data_13 <= {13'd0, image_algo_mode};
        // [0x000E] 二值化阈值回读 (assigned as wire)
        // fsmc_tx_data_14 <= {8'd0, image_threshold};
        fsmc_tx_data_15 <= 16'd0; // Reserved
        fsmc_tx_data_16 <= debug_data[15:0];  // 0x0010: 帧计数器的低16位 (Frame Cnt)
        fsmc_tx_data_17 <= debug_data[31:16]; // 0x0011: 有效像素计数低16位
        fsmc_tx_data_18 <= debug_data[47:32]; // 0x0012: 有效像素计数高16位
        fsmc_tx_data_19 <= 16'hFFFF;          // 0x0013: 调试位标记 (Reserved)
    end
end
...
assign exio[3:1] = image_algo_mode;   // 图像处理模式指示

//------------------------------------------
// 5. 图像处理模式控制 (长按切换 或 FSMC控制)
//------------------------------------------
// 图像处理模式: 
// 000=原始, 001=灰度, 010=二值, 011=Sobel, 100=腐蚀, 101=膨胀
reg [2:0] image_algo_mode;
reg [7:0] image_threshold;  // 二值化阈值

// FSMC RX寄存器同步到 clk_50m (用于图像处理控制)
// fsmc_rx_data_3: 图像处理算法模式 bit[2:0]
// fsmc_rx_data_4: 二值化阈值 bit[7:0]
// fsmc_rx_data_3[15]: STM32图像处理控制使能 (1=FSMC控制, 0=长按控制)
(* ASYNC_REG = "TRUE" *) reg [15:0] fsmc_image_ctrl_sync0, fsmc_image_ctrl_sync1;
(* ASYNC_REG = "TRUE" *) reg [15:0] fsmc_image_thresh_sync0, fsmc_image_thresh_sync1;

wire stm32_image_ctrl_enable = fsmc_image_ctrl_sync1[15];
wire [2:0] stm32_image_algo = fsmc_image_ctrl_sync1[2:0];
wire [7:0] stm32_image_threshold = fsmc_image_thresh_sync1[7:0];

always @(posedge clk_50m or negedge rst_n) begin
    if (!rst_n) begin
        fsmc_image_ctrl_sync0 <= 16'd0;
        fsmc_image_ctrl_sync1 <= 16'd0;
        fsmc_image_thresh_sync0 <= 16'd128;
        fsmc_image_thresh_sync1 <= 16'd128;
    end else begin
        fsmc_image_ctrl_sync0 <= fsmc_rx_data_3;
        fsmc_image_ctrl_sync1 <= fsmc_image_ctrl_sync0;
        fsmc_image_thresh_sync0 <= fsmc_rx_data_4;
        fsmc_image_thresh_sync1 <= fsmc_image_thresh_sync0;
    end
end

// 长按边缘检测 (用于切换模式)
reg long_press_d0, long_press_d1;
wire long_press_rising = long_press_d0 && !long_press_d1;
...
always @(posedge clk_50m or negedge rst_n) begin
    if (!rst_n) begin
        ...
        long_press_d0 <= long_press;
        long_press_d1 <= long_press_d0;
    end
end
...
always @(posedge clk_50m or negedge rst_n) begin
    if (!rst_n) begin
        image_algo_mode <= 3'd0;   // 默认原始图像
        image_threshold <= 8'd128; // 默认阈值
    end else begin
        if (stm32_image_ctrl_enable) begin
            // FSMC控制模式: 直接使用STM32设置的值
            image_algo_mode <= stm32_image_algo;
            image_threshold <= stm32_image_threshold;
        end else begin
            // 长按控制模式
            if (long_press_rising) begin
                // 循环切换 6 种模式 (0-5)
                if (image_algo_mode >= 3'd5)
                    image_algo_mode <= 3'd0;
                else
                    image_algo_mode <= image_algo_mode + 1'b1;
            end
        end
    end
end
...
fmc_control fmc_control1(
     ...
     .write_data_16  (fsmc_tx_data_16),
     .write_data_17  (fsmc_tx_data_17),
     .write_data_18  (fsmc_tx_data_18),
     .write_data_19  (fsmc_tx_data_19),
     // 接收寄存器
...
ov5640_rgb565_lcd u_ov5640 (
    ...
    // 图像处理控制
    .image_algo_mode (image_algo_mode),  // 图像处理算法模式
    .image_threshold (image_threshold)   // 二值化阈值
    );
