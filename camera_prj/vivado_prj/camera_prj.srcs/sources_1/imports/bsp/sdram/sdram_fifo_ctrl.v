...
reg        safe_read_bank;               // [NEW] 安全读取Bank (已完成写入的Bank)
...
always @(posedge clk_ref or negedge rst_n) begin
    if (!rst_n) begin
        ...
        sw_bank_en <= 1'b0;
        rw_bank_flag <= 1'b0;
        safe_read_bank <= 1'b1;  // [NEW] 初始读Bank为1，写Bank为0
    end
    else if(wr_load_flag) begin              //检测到写端口复位信号时，写地址复位
        rw_bank_flag <= ~rw_bank_flag;       // 切换 Bank (恢复乒乓操作)
        sdram_wr_addr <= {~rw_bank_flag, wr_min_addr[19:0]}; // 写地址指向新 Bank
        sw_bank_en <= 1'b0;
    end
        ...
        if(sdram_pingpang_en) begin          //SDRAM 读写乒乓使能
            ...
            else begin                       //完成一帧写入，切换BANK
                rw_bank_flag <= ~rw_bank_flag;   
                safe_read_bank <= rw_bank_flag;  // [NEW] 刚写完的Bank变为安全读取Bank
                sw_bank_en <= 1'b1;
            end            
        end       
                                             //若突发写SDRAM结束，更改写地址
...
always @(posedge clk_ref or negedge rst_n) begin
    if(!rst_n) begin
        ...
        if(sdram_pingpang_en)                // [FIX] 使用安全读取Bank，而非实时写入Bank
            sdram_rd_addr <= {safe_read_bank, rd_min_addr[19:0]};
        else
