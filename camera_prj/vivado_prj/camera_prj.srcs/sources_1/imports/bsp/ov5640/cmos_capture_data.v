...
module cmos_capture_data(
    ...
    output  reg  [12:0]   cmos_h_pixel_o     // [NEW] 实时测量的行宽
    );
...
always @(posedge cam_pclk or negedge rst_n) begin
    if(!rst_n) begin
        h_cnt <= 13'd0;
        cmos_h_pixel_o <= 13'd0;
    end else if(cam_href) begin
        // 每生成一个有效的 16位 像素 (byte_flag=1时完成组包)
        if(byte_flag)
            h_cnt <= h_cnt + 1'b1;
    end else begin
        // HREF 下降沿 (行结束)，更新测量值 (只有当计数值非0时更新，避免噪声)
        if(h_cnt > 13'd0)
            cmos_h_pixel_o <= h_cnt;
        h_cnt <= 13'd0; // 复位计数器等待下一行
    end
end
