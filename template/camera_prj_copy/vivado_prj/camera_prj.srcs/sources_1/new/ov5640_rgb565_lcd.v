...
reg [15:0] reset_counter; // [MODIFIED] 增加位宽以延长复位时间
...
always @(posedge clk_100m or negedge sys_rst_n) begin
    if(!sys_rst_n) begin
        ...
        // 初始上电复位跟随 system reset
        if(rst_n == 1'b0) begin
             i2c_config_rst_n <= 1'b0;
             config_state <= 2'd0;
        end 
        // 模式切换检测 -> 触发复位
        else if(mode_change_pulse) begin
             i2c_config_rst_n <= 1'b0;
             reset_counter <= 16'd0;
             config_state <= 2'd1; // Reset active state
        end
        // 复位保持状态
        else if(config_state == 2'd1) begin
             if(reset_counter < 16'd50000) // 500us @ 100MHz, 足够覆盖 I2C 时钟周期
                 reset_counter <= reset_counter + 1'b1;
             else begin
                 i2c_config_rst_n <= 1'b1; // 释放复位
                 config_state <= 2'd0;     // 回到空闲
             end
        end
        else begin
             i2c_config_rst_n <= 1'b1;
        end
    end
end
...
i2c_ov5640_rgb565_cfg u_i2c_cfg(
    .clk                (i2c_dri_clk),
    .rst_n              (i2c_config_rst_n), // [MODIFIED] 使用受控复位信号
...
cmos_capture_data u_cmos_capture_data(      //系统初始化完成之后再开始采集数据
    ...
    .cmos_frame_vsync   (),
    .cmos_frame_href    (),
    .cmos_frame_valid   (wr_en_int),            //数据有效使能信号
    .cmos_frame_data    (wr_data_int)           //有效数据
    );
