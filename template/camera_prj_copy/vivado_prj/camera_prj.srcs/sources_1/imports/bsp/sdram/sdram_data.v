...
(
    ...
    input   [MEM_DATA_BITS - 1:0]    sdram_data_in,    //写入SDRAM中的数据
    output  [MEM_DATA_BITS - 1:0]    sdram_data_out,   //从SDRAM中读取的数据
    input   [ 3:0]    work_state,	    //SDRAM工作状态寄存器
    input   [ 9:0]    cnt_clk, 		    //时钟计数
    
    inout   [MEM_DATA_BITS - 1:0]    sdram_data		//SDRAM数据总线
    );
...
reg [MEM_DATA_BITS - 1:0] sdram_din_r;	                //寄存写入SDRAM中的数据
reg [MEM_DATA_BITS - 1:0] sdram_dout_r;	            //寄存从SDRAM中读取的数据
...
assign sdram_data = sdram_out_en ? sdram_din_r : {MEM_DATA_BITS{1'bz}};
...
always @ (posedge clk or negedge rst_n) begin
	if(!rst_n) 
        sdram_din_r <= {MEM_DATA_BITS{1'b0}};
	else if((work_state == `W_WRITE) | (work_state == `W_WD))
...
always @ (posedge clk or negedge rst_n) begin
	if(!rst_n) 
        sdram_dout_r <= {MEM_DATA_BITS{1'b0}};
